...
- **Pixel-art SNES aesthetic** throughout. The user has invested heavily
  in this and loves it. Honor it.
...
| File | Role |
|---|---|
| `nav-logo.png` | Header brand mark (horizontal lockup) |
| `keeper-hq-logo.png` | Old standalone shield — UNUSED, safe to delete |
...
- **Nav logo**: `src/App.jsx` header (height 56)
- **Sport sprites**: `<SportLogo>` from `src/components.jsx` reads
  `SPORT_CONFIG[sport].logo` → `/sport-{sport}.png`
  - Home cards (`HomeView.jsx`): 44px inline next to title
  - League detail header (`LeagueView.jsx`): 96px next to title
...
- `public/keeper-hq-logo.png` — unused since nav swap. Safe to delete.
