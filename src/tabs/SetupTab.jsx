          ...
          <div style={{ padding: '10px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {dropped.map((p, i) => (
              <div key={i}
                ...
                <button onClick={() => undropPlayer(i)} style={{ background: 'rgba(76,175,125,0.12)', border: 'none', borderRadius: 4, padding: '3px 10px', fontSize: '11px', fontWeight: 600, color: tokens.success, cursor: 'pointer', fontFamily: 'inherit' }}>↶ Undo</button>
              </div>
            ))}
          </div>
        </div>
                ...
                {activeKeepers.map((k, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 11px', background: t.sectionBg, border: `1px solid ${t.border}`, borderRadius: 8 }}>
                    <div>
                      ...
                      {k.isNew && <span style={{ fontSize: '10px', color: tokens.success, marginLeft: 8, fontWeight: 700, background: 'rgba(76,175,125,0.15)', padding: '2px 6px', borderRadius: 10 }}>NEW</span>}
                    </div>
